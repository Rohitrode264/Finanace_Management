...
// ── User Management ───────────────────────────────────────────────────────────
router.get('/users', permissionMiddleware(['MANAGE_USERS', 'CREATE_USER'], 'USER'), (req, res) => rbacController.listUsers(req, res));
router.post('/users', permissionMiddleware(['MANAGE_USERS', 'CREATE_USER'], 'USER'), (req, res) => rbacController.createUser(req, res));
...
router.delete('/roles/:id/permissions/:permId', permissionMiddleware('MANAGE_PERMISSIONS', 'PERMISSION'), (req, res) => rbacController.revokePermission(req, res));
